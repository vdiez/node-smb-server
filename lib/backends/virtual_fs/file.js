          ...
          return this.get_storage(root.storage_id)
              .then(storage => {
                ...
                return connection.stat(path.posix.join(root.dirname, uri), storage)
                    ...
                    .then(stats => cb(null, new FSFile(filePath, stats, tree)))
                    .catch(err => cb(SMBError.fromSystemError(err, 'unable to create file due to unexpected error ' + filePath)))
              });
        }
        return {
          name: '/',
